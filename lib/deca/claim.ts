 ...
 */
export async function claimDeca(token: string, companyId: string, userId: string): Promise<{ decaId: string }> {
  ...
  if (claim.expiresAt.getTime() < Date.now())
    throw new ClaimError("expired", "El enlace de guardado ha caducado. El DeCA sigue siendo válido y accesible por su URL.");
