 ...
 * anonymous session. Automates the steps that CAN be automated; the QR scan
 * from a second physical device is in `docs/production-smoke-checklist.md`.
 *
 * D-060 (owner directive) restored the original "no signup wall" design after
 * D-052/PRIORITY 1 had temporarily required a full account for every DeCA: an
 * anonymous visitor generates their FIRST DeCA with just a name + email
 * (`lib/deca/lead.ts`); registering afterward is an upsell (claim), never a
 * precondition. The SAME browser is sent to registration for a SECOND
 * anonymous DeCA (`app/crear/page.tsx`'s `fvd_lead` cookie gate).
 *
 * Launch blockers this spec fails on: mock/fake document, QR pointing at the
 * wrong host, `/d/[token]` returning HTML, missing legal fields, PDF changing
 * between fetches, claim regenerating the document.
 */
...
test.describe("LAUNCH #20 — production happy path", () => {
  test("anonymous → PDF → QR-equivalent → save account (claim) → duplicate", async ({
    ...
    // 5-6. Generate once → result screen, no account required
    ...
    // D-053: the claimed account still needs a verified email to generate the
    // duplicate below — verify for real via the token the email carried.
    const claimBody = await claimRes.json();
    await page.request.get(`/verificar-email/${claimBody.verifyTestToken}`);
