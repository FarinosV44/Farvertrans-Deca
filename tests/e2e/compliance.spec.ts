 ...
 * register one and verify it for real via the token. Used here so this
 * suite's documents are company-owned, which several R-nn checks assume.
 */
...
test.describe("R-1…R-13 compliance", () => {
  test("R-3/R-4: the PDF is native text (not a scan) and ≤ 5 MB", async ({ request }) => {
    ...
    for (const v of [
      ...
      payload.loadLocation.name,
      payload.loadLocation.address,
      payload.loadLocation.postalCode,
      payload.loadLocation.city,
      payload.unloadLocation.name,
      payload.unloadLocation.address,
      payload.unloadLocation.postalCode,
      payload.unloadLocation.city,
      payload.loadDate,
      payload.unloadDate,
      payload.goods,
      payload.weight,
