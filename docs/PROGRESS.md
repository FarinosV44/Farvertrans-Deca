...
- Branches: integration branch `develop`; committing BUILD slices directly to `develop`. **`develop` and `main` are BOTH at `fdaaaa0` (2026-09-12, D-241 — user's explicit instruction "push to main verify no gap and no new migrations") — fully in sync.** Carries D-226 through D-248: #128/#129/#130 (P1), #132/#133/#135/#136/#137/#139/#140/#141-partial (P2 audit fixes), #138 (P1 live production fix, 3-component sweep), #131 mobile UX fix + its correction, and #112/#119's ACLARACIÓN FINAL corrections (which REPLACE this session's own earlier D-229/D-230 work on those same issues — read the full issue comment history before ever touching #112/#119 code again, both have been substantially rewritten more than once this session). `develop` is now AHEAD of `main` (last synced at D-241/`fdaaaa0`) by D-242 through D-246 — none of these have been pushed to `main`, only committed to `develop` (some not yet even pushed to `origin/develop`); merging to `main` needs an explicit future user instruction. Remaining P2: #141's remaining scope (empresa, datos/saved-data-manager, deca/[id] cockpit cluster — see D-246), stale threat-model.md. All P3 audit findings NOT started — that's the next work after #141. `git merge`/`checkout` commands were initially blocked by the Claude Code auto-mode permission classifier ("Modify Shared Resources") this session; the user approved throughout. Product version is **0.3.0**; UI supports 9 locales incl. `pt`. No tag requested. **Production DB schema is current through D-241** (verified directly via `prisma migrate status` against production both before and after applying) — **production APP CODE is still NOT redeployed** for any of D-202→D-240; still runs a pre-`cc82787` build reporting `0.2.0`. The user's next Hostinger redeploy picks up everything at once — **and needs the same targeted `contentItem.update` this session ran on dev** to refresh the live guide's already-seeded body (a plain redeploy does not do this — see D-210). **Production DB credentials have now been pasted in chat multiple times this session (D-235, D-241)** — the user says they will rotate; the long-overdue DB password/anon-key rotation (flagged since D-158) is now the single most urgent outstanding item, not yet done.
...
## Current position
- **D-248 — #141 continued: `app/panel/datos` + `saved-data-manager.tsx` localized — the largest
  remaining piece, this session (2026-09-13).** Full `panel.datos` dictionary section added to all 9
  locales; `DatosHabitualesManager` reads `useT()` once and threads the resolved messages down to
  `TabPanel`/the 4 per-kind forms as a plain prop (safe — sibling client functions, not a Server→Client
  crossing). Verified via production build + curl smoke test before e2e. Gate: tsc/eslint/prettier
  clean, 501/501 unit unaffected, `datos-habituales-rutas`+`favorites`+`master-data`+`workspace`
  16/16 (incl. the `/panel/datos` a11y check), `panel-nav.spec.ts` 6/6. Committed to `develop`, not
  yet pushed. **Issue #141 remaining scope: only the `deca/[id]` cockpit page + its 5 dependents
  (`save-template.tsx`, `doc-summary.tsx`, `qr-card.tsx`, `version-timeline.tsx`,
  `availability-notice.tsx`).** Full detail in `docs/decisions.md` D-248.
- **D-247 — #141 continued: `app/panel/empresa` + `CompanyProfileForm` + `CompanyLogoManager`
  localized, this session (2026-09-13).** Applied the D-246 lesson proactively — both client
  components use `useT()` directly rather than a server-passed prop, since `CompanyLogoManager`'s
  messages include functions. Verified via production build + direct curl smoke test BEFORE running
  e2e this time. Gate: tsc/eslint/prettier clean, 501/501 unit unaffected, `company-logo.spec.ts`
  7/7, `panel-company-completeness.spec.ts` 1/1, `panel-nav.spec.ts` 6/6. Committed to `develop`, not
  yet pushed. **Remaining #141 scope:** `app/panel/datos` + `saved-data-manager.tsx` (~1060 lines,
  largest remaining piece), the `deca/[id]` cockpit page + 5 dependents. Full detail in
  `docs/decisions.md` D-247.
- **D-246 — #141 [P2 audit finding, i18n] partial fix: `equipo`/`plantillas`/`integraciones` now
  route through the dictionary system across all 9 locales, this session (2026-09-12). A real RSC
  serialization bug (function props can't cross Server→Client boundary) was found and fixed along the
  way — `TeamManager`/`IntegrationRequestForm` now use `useT()` like `wizard.tsx` does, instead of
  receiving a resolved dictionary as a prop. Full detail in `docs/decisions.md` D-246, including a
  scope correction (corregir/inspeccion are NOT gaps — wizard is already localized, inspeccion is
  intentionally Spanish-only by design). Gate: tsc/eslint/prettier clean, 501/501 unit unaffected,
  production build clean, `team.spec.ts` 12/12, `panel-nav.spec.ts`+`membership.spec.ts`+
  `admin-growth.spec.ts` 16/16. Committed to `develop`, not yet pushed. **Remaining #141 scope:**
  `app/panel/empresa` + `CompanyProfileForm`/`CompanyLogoManager`; `app/panel/datos` +
  `saved-data-manager.tsx` (~1060 lines, largest remaining piece); the `deca/[id]` cockpit page +
  `save-template.tsx`/`doc-summary.tsx`/`qr-card.tsx`/`version-timeline.tsx`/`availability-notice.tsx`.
- **D-245 — #140 [P2 audit finding, accessibility] fixed: the shared `Modal` had no focus trap and no
  focus restoration (WCAG 2.2 §2.4.3), this session (2026-09-12). Full detail in `docs/decisions.md`
  D-245.** `components/app/modal.tsx` focused the first field on open but a plain Tab pass could
  escape past the last field into the page behind the still-open backdrop, and closing never returned
  focus to whatever opened it. Fixed with Tab/Shift+Tab cycling inside the dialog's focusable set plus
  `openerFocused?.focus()` on cleanup. New e2e test on the "Añadir vehículo" modal; verified
  red-then-green via `git stash`. Gate: tsc/eslint/prettier clean (2 pre-existing unrelated warnings
  only), 501/501 unit unaffected, 6/6 `datos-habituales-rutas.spec.ts`. Committed to `develop`, not
  yet pushed.
...
- **D-241 — `develop` merged to `main` through D-240, pending migration applied to production, this
  ...
  empty. `prisma migrate status` against production found exactly the 1 expected pending migration
  (`20260912180000_availability_preferred_destination_country`, D-240's own); applied via `migrate
  deploy`, confirmed up to date after. No RLS re-enrollment needed (no new table). **Outstanding:**
  production app code still not redeployed; DB credential rotation is now more overdue, having been
  pasted into chat again this session.
