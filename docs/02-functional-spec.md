...
- **Events:** `landing_view`, `click_crear_deca`, `signup_started`, `signup_completed`, `deca_started`, `deca_generated`, plus `deca_shared`, `deca_corrected`, `claim_completed`. Each row: event, timestamp, session id (first-party), path, `ref`/UTM snapshot, app version. No PII in events.
