...
| T-5 | Injection (SQL, XSS) | Prisma parameterized queries only; React auto-escaping; `zod` validation on every input; no `dangerouslySetInnerHTML` with user data; CSP header (`middleware.ts`) | IN PLACE — `zod` on every route, CSP + security headers via `middleware.ts`, verified in `tests/e2e/launch-gate.spec.ts`. CSP is `script-src 'self' 'unsafe-inline'` (no external scripts, no `unsafe-eval` in prod); nonce-based CSP is a tracked post-launch item |
...
| Data loss (disk failure, Supabase account loss, a bad migration, an errant `rm`) | The whole product is one Supabase project; there is no delete path in code (D-067) but the project itself is a single point of failure | **#60 / D-119:** `scripts/backup.mjs` + `.github/workflows/backup.yml` take a daily `age`-encrypted archive of Postgres + the PDF bucket to an independent S3-compatible store (separate credentials). RPO ≤24h, RTO ≤4h. Restore is rehearsed (`scripts/restore.mjs`, refuses a production target) and logged in `docs/07-release.md` §6. Full design: `docs/backup-and-restore.md` |
| Billing / financial data exposure (FUTURE — not built) | If a paid tier is added, `Subscription`/`Invoice`/`Payment` rows are personal + financial data | Designed but not built (#64 / D-120, `docs/design/billing-model.md`): no card data ever stored (only provider references), owner-only access, append-only invoices, `SubscriptionEvent` + `SecurityAuditLog`. Re-do this row properly when the feature is greenlit |
