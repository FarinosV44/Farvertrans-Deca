...
| c2 | Lugar de destino — **empresa/establecimiento + dirección completa** | `unloadLocation` (same shape; `province` optional) | `#unloadLocationName/Address/PostalCode/City/Province/Country` — "Lugar de descarga" | `LUGAR DE DESCARGA` (address built only from informed fields) | `deca-validate.test.ts` (missing unloadLocation subfields), `deca-location.test.ts`, `compliance.spec.ts` R-3 |
...
- **Party postal code + población (D-145).** `postalCode` / `city` are captured
  ...
  also carries `postalCode` / `city` (#85 added them, migration
  `20260908170252_saved_company_postal_city`; #86 p2 made them **mandatory** on
  the habitual — a saved contact always stores a full domicilio), so picking a
  saved company from a wizard autofill dropdown fills the party's "CP población"
  line too. The DeCA party fields themselves stay optional (D-145) — #86 p2 is
  scoped to the habituales master data, not the document form.
