...
| POST /api/saved/[kind] | route | app/api/saved/[kind]/route.ts | docs/reference/endpoints.md | Create a saved company (with role)/vehicle (with alias)/location (structured, load/unload/both), scoped to the company, any non-`read_only` member (PRODUCT #56); 422 on invalid |
| DELETE /api/saved/[kind]/[id] | route | app/api/saved/[kind]/[id]/route.ts | docs/reference/endpoints.md | Delete a saved entity (company-scoped, any non-`read_only` member, PRODUCT #56); never touches generated DeCA |
...
| searchCompanyDecas() | function | lib/data/search.ts | docs/reference/lib.md | Company-scoped global search (PRODUCT #56) — reuses `listHistory`'s `q` free-text filter, top 8 hits with a label/sub/href shape for the command palette |
...
| lib/team.ts (createInvite / acceptInvite / consumeInviteToken / listMembers / removeMember / changeRole / canWrite / getInvitePreview / TeamError) | module | lib/team.ts | docs/reference/lib.md | Company workspaces + invitations (TEAM #27/#37, PRODUCT #56); owner=Company Admin / member=Operator / read_only=Auditor (view-only, `canWrite()` gates every mutating route); invite tokens stored hashed, 14-day TTL, one-time; changeRole() promotes/demotes and never drops the last admin (any non-owner target role, not just member) |
...
| POST /api/deca/[id]/version | route | app/api/deca/[id]/version/route.ts | docs/reference/endpoints.md | Correct a DeCA → new version (R-13); any authed non-`read_only` company member (PRODUCT #56); 404/403/422 |
