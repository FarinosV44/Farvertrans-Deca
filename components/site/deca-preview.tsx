    ...
    >
      <div className="rounded-[var(--radius-md)] bg-white p-4">
        ...
        <div className="mt-3 space-y-2">
          {["Cargador — NIF y domicilio", "Transportista efectivo — NIF", "Origen → Destino"].map((l) => (
            <div key={l} className="rounded-[6px] border border-[var(--color-border)] px-3 py-2 text-xs text-[var(--color-text-muted)]">
              {l}
            </div>
          ))}
        </div>
      ...
      <div className="mt-3 flex items-center gap-3 rounded-[var(--radius-md)] bg-white p-3">
        <span className="grid h-9 w-9 place-items-center rounded-full bg-[var(--color-success)] text-white">✓</span>
        <div className="flex-1">
          <p className="text-sm font-bold">DeCA generado</p>
          <p className="text-xs text-[var(--color-text-muted)]">PDF nativo · QR · URL de descarga directa</p>
        </div>
        <div className="grid h-12 w-12 grid-cols-4 grid-rows-4 gap-[2px] rounded-[4px] bg-white p-1 ring-1 ring-[var(--color-border)]">
          {Array.from({ length: 16 }).map((_, i) => (
            <span key={i} className={(i * 7) % 3 === 0 ? "bg-[var(--color-text)]" : "bg-transparent"} />
          ))}
        </div>
      </div>
    </div>
